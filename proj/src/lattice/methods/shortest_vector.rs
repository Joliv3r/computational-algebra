...
impl Lattice {
    ...
    pub fn shortest_vector_by_enumeration(&self) -> Result<(Array1<f64>, Vec<(f64, f64)>), String> {
        ...
        for x_n in 0..bound_n {
            ...
            let (candidate_vector, candidate_length) = self.shortest_vector_enumeration_steps(1, &combination, &shortest_vector.to_vec().into(), shortest_length, &mut checked_points);
    ...
    fn shortest_vector_enumeration_steps(&self, depth: usize, combination: &Vec<i64>, current_shortes_vector: &Array1<f64>, current_shortest_length: f64, checked_points: &mut Vec<(f64, f64)>) -> (Array1<f64>, f64) {
        if depth == self.columns() {
            let current_vector = self.get_lattice_point(&combination.iter().rev().cloned().collect_vec()).expect("Should exist.");
        ...
        for i in lower_bound..=upper_bound {
            ...
            let (candidate_vector, candidate_length) = self.shortest_vector_enumeration_steps(depth+1, &new_combination, &shortest_vector, shortest_length, checked_points);
            if candidate_length < shortest_length && candidate_length != 0. {
                shortest_vector = candidate_vector;
                shortest_length = candidate_length;
